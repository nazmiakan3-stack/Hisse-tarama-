...
def scan_bist_stocks(symbol_list, scan_time):
    print(f"[{datetime.now().strftime('%H:%M:%S')}] {len(symbol_list)} Yan Tahta İçin {scan_time} Taraması Başlatıldı...")
    ...
    for symbol in symbol_list:
        ...
        # 🚀 GÜNLÜK TAVAN ADAYI
        if rsi >= RSI_MOMENTUM_LIMIT and change >= CHANGE_MOMENTUM_LIMIT and rec in ["STRONG_BUY", "BUY"]:
    ...
    print(f"[{datetime.now().strftime('%H:%M:%S')}] {scan_time} Taraması Bitti. Toplam {match_count} adet fırsat bildirildi.")

# ============================================================
# ANA ÇALIŞMA DÖNGÜSÜ
# ============================================================
def main():
    send_telegram_msg("🤖 <b>TRADINGVIEW 470 YAN TAHTA BİST BOTU V8 AKTİF!</b>\n⏰ Özel Tarama Saatleri: <b>09:50</b> ve <b>10:10</b>\n📊 Toplam Taranan Yan Tahta: <b>~470 Adet</b>\n🔥 KAP Haberleri: Yalnızca Yüksek Değerli (4-5 Yıldız)")
    ...
    while True:
        try:
            ...
            if current_time in TARGET_SCAN_TIMES and scan_key not in SCANNED_TIMES_TODAY:
                ...
                send_telegram_msg(f"⏰ <b>[{current_time} SEANS TARAMASI BAŞLADI]</b>\nToplam {len(target_symbols)} adet yan tahta taranıyor...")
                scan_bist_stocks(target_symbols, current_time)
                SCANNED_TIMES_TODAY.add(scan_key)
                send_telegram_msg(f"✅ <b>[{current_time} SEANS TARAMASI COMPLETED]</b>")
